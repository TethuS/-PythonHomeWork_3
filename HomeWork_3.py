...
text_list = list(some_text.split()) # Разбиваем текст по словам на отдельный элементы Списка
print(text_list)
...
# Заданиче 4. Получить из list п.3 dict, Ключи - слова, а Значения - кол-во появлений в тексте
words_count = {}

for i in range(len(new_text_list)): # Проходимся по Списку слов с нижним регистром
    # Создаем Ключ соответсвующий слову из Списка, а методом .count(), присваиваем кол-во элементов в Значение
    words_count[new_text_list[i]] = new_text_list.count(new_text_list[i])
print(words_count)

# Задание 5. Вывести 5 наиболее часто встречающихся слов (sort) и кол-во разных слов в тексте (set)
